...
public class BodySignUp extends JPanel {
    ...
    JTextField text_email = new JTextField(15);
    ...
    public BodySignUp(Body body) {
        ...
        btn_signup.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
                if(text_id.getText().isBlank() || text_pw.getText().isBlank() || text_name.getText().isBlank()  || text_phone.getText().isBlank() || text_email.getText().isBlank()
                        || text_address.getText().isBlank()) {
                    msg_error.setText("빈칸을 입력해 주세요.");
                } else {
                    Member member = new Member(text_id.getText(),text_pw.getText(), text_name.getText(),
                            text_phone.getText(), text_email.getText(), text_address.getText());
                    System.out.println("test");
                    MemberApi.signUp(member);
                    JOptionPane.showMessageDialog(null, "회원가입이 완료되었습니다.", "Cookeryket", JOptionPane.INFORMATION_MESSAGE);
                    body.showLogin();
        ...
        text_id.addKeyListener(new KeyAdapter() {
            public void keyTyped(KeyEvent e) {
                if(!String.valueOf(e.getKeyChar()).matches("^[0-9a-zA-Z]*$")) {
                    e.consume();
                    msg_error.setText("아이디는 영문과 숫자만 입력 가능합니다.");
        ...
        text_name.addKeyListener(new KeyAdapter() {
            public void keyTyped(KeyEvent e) {
                if(!String.valueOf(e.getKeyChar()).matches("^[가-힣]*$")) {
                    e.consume();
                    msg_error.setText("이름은 한글만 입력 가능합니다.");
                } else { msg_error.setText(""); }
            }});
        text_phone.addKeyListener(new KeyAdapter() {
            public void keyTyped(KeyEvent e) {
                if(!String.valueOf(e.getKeyChar()).matches("^[-0-9]*$")) {
                    e.consume();
                    msg_error.setText("전화번호는 숫자와 하이픈만 입력 가능합니다.");
                } else { msg_error.setText(""); }
            }});
